...
const addReview = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({
        message: "Review added and supplier rating updated successfully.",
        review: newReview,
      });
  } catch (err) {
    ...
    res
      ...
      .json({
        message:
          "An error occurred while adding the review and updating the supplier rating.",
        error: err.message,
      });
  }
};
...
const getReviewsBySupplierID = async (req, res) => {
  ...
  try {
    ...
    if (reviews.length === 0) {
      return res
        .status(404)
        .json({ message: "No reviews found for this supplier." });
    }
...
module.exports = { getAllReviews, addReview, getReviewsBySupplierID };
